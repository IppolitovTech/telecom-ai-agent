...
- [ ] `rag/embeddings.py` — обёртка над моделью эмбеддингов (`multilingual-e5-small` для русскоязычного корпуса)
- [ ] `rag/ingest.py` — chunking (500–800 токенов, overlap 50–100) + запись в Chroma
- [ ] `rag/retriever.py` — `retrieve(query, k=4)`, возвращает чанки вместе с метаданными: имя документа, номер чанка, score
- [ ] Триггер ingest явно зафиксирован: `python -m rag.ingest` запускается один раз при старте backend и обрабатывает всё, что лежит в `knowledge/` (не на каждый запрос — иначе время старта сервера растёт). `POST /api/upload` (Этап 1) после сохранения файла должен вызывать ту же ingest-функцию для нового файла, а не только класть его в папку — иначе он не попадёт в поиск
...
- [ ] `agent/router.py`: LLM-классификатор intent — `rag_search` / `tool_call` / `direct_answer` / `combined`
- [ ] `agent/tools.py`: `check_order_status(order_id)`, `create_ticket(topic)` — заглушки с фиктивными данными
- [ ] Ветка `rag_search` → вызов `retriever.retrieve()`
- [ ] Ветка `tool_call` → вызов нужного tool по имени и аргументам, извлечённым классификатором
- [ ] Ветка `combined` → ручной ReAct-цикл (не `AgentExecutor`): максимум 2–3 итерации `think → act → observe` в обычном Python `while`-цикле — модель на каждом шаге решает, каких данных не хватает (RAG или tool), вызывает нужное действие, получает результат и решает, достаточно ли информации для финального ответа
- [ ] Жёсткий лимит итераций (например, 3) — защита от зацикливания
- [ ] Финальный вызов `ChatAnthropic(model=settings.anthropic_model)` (актуальная модель, например `claude-sonnet-5`, — через `.env`/`Settings`, без хардкода строки в `agent/`) — синтезирует найденный контекст/результат tool в связный ответ
- [ ] Память диалога по `session_id` (in-memory dict — простое хранилище, обнуляется при рестарте, это осознанный компромисс для pet-проекта)
- [ ] Если RAG не находит релевантного контекста — агент честно говорит об этом, не выдумывает
- [ ] Логирование ключевых шагов через встроенный `logging` (JSON-формат, без отдельных библиотек вроде structlog) — `session_id`, выбранный intent, тайминги (`llm_ms`, `retrieval_ms`) для каждого шага
