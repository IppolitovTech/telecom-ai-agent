...
```
- [x] `sources` заполняется из метаданных, которые уже возвращает `retriever.retrieve()` (Этап 2)
- [x] Поля `tool_calls` и `sources` присутствуют всегда, даже если пустые списки
...
- [x] README со схемой из ARCHITECTURE.md, стеком, инструкцией запуска
- [x] Явно указать retrieval accuracy из Этапа 6 (конкретное число)
- [x] Упоминание использования AI-ассистентов в разработке
